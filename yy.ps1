...
function Get-Html3ChannelFragment {
    ...
    foreach ($entry in @($entries.Values | Where-Object { [string]$_.availability -notin @('subscriber_only', 'private', 'premium_only') -and [long]$_.timestamp_ms -ge $CutoffMs } | Sort-Object {[long]$_.timestamp_ms} -Descending)) {
        ...
        $age = Format-RelativeVideoTime ([long]$entry.timestamp_ms)
        $ageClass = if ($age -match '^[12] weeks? ago$') { ' video-age-week' } else { '' }
        $card = '<article class="card"><a class="video-link" href="{2}" target="_blank" rel="noopener noreferrer"><div class="preview"><img src="{0}" alt=""></div><div class="video-title">{1}</div></a><div class="video-age{9}">{3}</div><div class="checks"><label><input class="y1" data-url="{2}" data-path="{4}" data-channel-id="{5}" data-video-id="{6}" type="checkbox"{7}> y1</label><label><input class="y2" data-url="{2}" data-path="{4}" data-channel-id="{5}" data-video-id="{6}" type="checkbox"{8}> y2</label></div></article>' -f ([System.Net.WebUtility]::HtmlEncode("https://i.ytimg.com/vi/$id/hqdefault.jpg")), ([System.Net.WebUtility]::HtmlEncode([string]$entry.title)), ([System.Net.WebUtility]::HtmlEncode($url)), ([System.Net.WebUtility]::HtmlEncode($age)), ([System.Net.WebUtility]::HtmlEncode('./' + $Channel)), ([System.Net.WebUtility]::HtmlEncode($ChannelId)), ([System.Net.WebUtility]::HtmlEncode($id)), $checkedY1, $checkedY2, $ageClass
