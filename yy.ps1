    ...
    while ($completed -eq 0) {
        foreach ($worker in $Workers) {
            ...
            if (Test-Path -LiteralPath $worker.Stderr) {
                $lines = @(Get-Content -LiteralPath $worker.Stderr -ErrorAction SilentlyContinue)
                for ($i = $worker.StderrCount; $i -lt $lines.Count; $i++) {
                    if ($lines[$i] -match '^(\[[^]]+\]|ERROR:|WARNING:|Aborting )' -and
                        $lines[$i] -notmatch '^\[debug\]') {
                        Write-Host "[@$($worker.Channel) scan] $($lines[$i])"
                    }
                }
                $worker.StderrCount = $lines.Count
            }
            if (-not $worker.Process.HasExited) {
                if (([System.DateTime]::UtcNow - $worker.LastHeartbeat).TotalSeconds -ge 15) {
                    $elapsed = [Math]::Floor(([System.DateTime]::UtcNow - $worker.Started).TotalSeconds)
                    Write-Host "[@$($worker.Channel) scan] Still running yt-dlp... $elapsed seconds elapsed"
                    $worker.LastHeartbeat = [System.DateTime]::UtcNow
                }
                continue
            }
    ...
    if (-not (Test-Path -LiteralPath $downloadedVideosFile)) {
        ...
        try {
            ...
            foreach ($item in @($value)) {
                [long]$epoch = 0
                if ($null -eq $item -or
                    [string]$item.channel_id -notmatch '^UC[A-Za-z0-9_-]+$' -or
...
function Add-DownloadedVideo {
    ...
    $items += [pscustomobject]@{
        ...
        video_id = $VideoId
        target = $Target
        download_epoch = [System.DateTimeOffset]::UtcNow.ToUnixTimeSeconds()
    }
    Save-DownloadedVideos $items
}
...
function Test-VideoSelection {
    ...
    if ([string]$Item.channel_id -notmatch '^UC[A-Za-z0-9_-]+$' -or [string]$Item.video_id -notmatch '^[A-Za-z0-9_-]+$') { return $false }
...
function Start-SelectedVideoHooks {
    ...
    foreach ($item in @($selected | Sort-Object @{ Expression = { if ($_.target -eq 'y2') { 0 } else { 1 } }; Ascending = $true })) {
        ...
        $Jobs[[guid]::NewGuid().ToString('N')] = [pscustomobject]@{
            Target = [string]$item.target; Url = [string]$item.url; DownloadPath = [string]$item.path; ChannelId = [string]$item.channel_id; VideoId = [string]$item.video_id; Hook = [string]$hook.Source; Order = $started; Process = $null; State = 'queued'; ExitCode = $null; OutputPath = ''; ErrorPath = ''; OutputLines = 0; ErrorLines = 0; Logs = @(); Succeeded = $false; Persisted = $false; LastProgressPercent = [double](-1); LastProgressBucket = -1
        }
...
function Test-ShouldEmitDownloadLog {
    param($Job, [string]$Line)

    if ($Line -notmatch '^\[download\]\s+([0-9]+(?:\.[0-9]+)?)%') { return $true }
    $percent = [double]::Parse($Matches[1], [System.Globalization.CultureInfo]::InvariantCulture)
    if ($Job.LastProgressPercent -ge 0 -and $percent -lt ($Job.LastProgressPercent - 1)) {
        # A second format (normally audio after video) restarts at zero.
        $Job.LastProgressBucket = -1
    }
    $bucket = [int][Math]::Floor($percent / $downloadProgressStepPercent)
    $emit = $bucket -gt $Job.LastProgressBucket -or $percent -ge 100
    $Job.LastProgressPercent = $percent
    if ($emit) { $Job.LastProgressBucket = $bucket }
    return $emit
}
        ...
        foreach ($logSource in @(@{ Path = $job.OutputPath; Count = 'OutputLines' }, @{ Path = $job.ErrorPath; Count = 'ErrorLines' })) {
            ...
            if ($lines.Count -gt $oldCount) {
                foreach ($line in @($lines[$oldCount..($lines.Count - 1)])) {
                    ...
                    if (-not (Test-ShouldEmitDownloadLog -Job $job -Line ([string]$line))) { continue }
        ...
        if ($job.State -eq 'running') {
            try {
                if ($job.Process.HasExited) {
                    ...
                    if ($job.State -eq 'completed' -and -not $job.Persisted) {
                        Add-DownloadedVideo $job.ChannelId $job.VideoId $job.Target
                        $job.Persisted = $true
                    }
                }
            }
...
function Write-Html3LoadingPage {
    ...
    $safeCheckpoint = [System.Net.WebUtility]::HtmlEncode('Checkpoint: ' + (Read-CheckpointMs))
    $page = @'
...
'@
    ...
    $page = $page.Replace('const refresh=async all=>{remember();setBusy(true);', 'const refresh=async all=>{remember();html3Failures=[];errors.textContent="";setBusy(true);')
