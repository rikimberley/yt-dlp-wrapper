...
generate_html() {
  ...
  for (( index = 1; index <= ${#channels}; ++index )); do
    ...
    if [[ -n "$cards" ]]; then
      channel_url=$(channel_url_for "$channel")
      print -r -- "<section class=channel><div class=channel-title><h2><a href=\"$(html_escape "$channel_url")\" target=_blank rel=\"noopener noreferrer\">$(html_escape "$channel")</a></h2><div class=controls><button data-action=y1 type=button>y1</button><button data-action=y2 type=button>y2</button><button data-action=none type=button>none</button></div></div><div class=grid>" >> "$tmp"
      print -r -- "$cards" >> "$tmp"
